const Cardsdata = [
    {
        id: 1,
<<<<<<< HEAD
        title: "The Great Gatsby",
        price: 109.95,
        description:
          "he Great Gatsby, Third novel by American author F. Scott Fitzgerald, published in 1925. Set in Jazz Age New York, it tells the tragic story of Jay Gatsby, a self-made millionaire, and his pursuit of Daisy Buchanan, a wealthy young woman whom he loved in his youth.",
        category: "Tragedy",
        image: "./img/png-transparent-leonardo-dicaprio-the-great-gatsbyl.png",
      },
      {
        id: 2,
        title: "Martin Eden ",
        price: 22.3,
        description:
          "Living in Oakland at the beginning of the 20th century, Martin Eden struggles to rise above his destitute, proletarian circumstances through an intense and passionate pursuit of self-education, hoping to achieve a place among the literary elite. His principal motivation is his love for Ruth Morse. Because Eden is a rough, uneducated sailor from a working-class background[5] and the Morses are a bourgeois family, a union between them would be impossible unless and until he reached their level of wealth and refinement.",
        category: "Roman",
        image:
          "https://upload.wikimedia.org/wikipedia/en/9/9b/MartinEden.jpg",
      },
      {
        id: 3,
        title: "master and margarita",
        price: 55.99,
        description:
          "The Master and Margarita combines supernatural elements with satirical dark comedy and Christian philosophy, defying categorization within a single genre. Many critics consider it to be one of the best novels of the 20th century, as well as the foremost of Soviet satires.",
        category: "	Fantasy, farce, supernatural, romance, satire, Modernist literature",
        image: "./img/cover.webp",
      },
      {
        id: 4,
        title: "Harry Potter",
        price: 15.99,
        description:
          "Harry Potter is a series of seven fantasy novels written by British author J. K. Rowling. The novels chronicle the lives of a young wizard, Harry Potter, and his friends Hermione Granger and Ron Weasley, all of whom are students at Hogwarts School of Witchcraft and Wizardry. The main story arc concerns Harry's struggle against Lord Voldemort, a dark wizard who intends to become immortal, overthrow the wizard governing body known as the Ministry of Magic and subjugate all wizards and Muggles (non-magical people).",
        category: "Fantasy ",
        image: "./img/Harry_Potter_and_the_Philosopher's_Stone_—_movie.jpg",
      },
      {
        id: 5,
        title:
          "war and peace",
        price: 695,
        description:
          "War and Peace (Russian: Война и мир, romanized: Voyna i mir; pre-reform Russian: is a literary work mixed with chapters on history and philosophy by the Russian author Leo Tolstoy. It was first published serially, then published in its entirety in 1869. It is regarded as one of Tolstoy's finest literary achievements and remains an internationally praised classic of world literature.",
        category: "novel",
        image: "./img/war.jpg",
      },
      {
        id: 6,
        title: "Crime and Punishment ",
        price: 168,
        description:
          "Crime and Punishment follows the mental anguish and moral dilemmas of Rodion Raskolnikov, an impoverished ex-student in Saint Petersburg who plans to kill an unscrupulous pawnbroker, an old woman who stores money and valuable objects in her flat. He theorises that with the money he could liberate himself from poverty and go on to perform great deeds, and seeks to convince himself that certain crimes are justifiable if they are committed in order to remove obstacles to the higher goals of 'extraordinary' men. Once the deed is done, however, he finds himself racked with confusion, paranoia, and disgust. His theoretical justifications lose all their power as he struggles with guilt and horror and confronts both the internal and external consequences of his deed.",
        category: "Novel, Psychological realism, Philosophical novel, crime genre",
        image: "./img/crime-and-punishment-by-fyodor-dostoyevsky.jpg",
      },
      {
        id: 7,
        title: "ANNA KARENINA",
        price: 9.99,
        description:
          "Anna Karenina is commonly thought to explore the themes of hypocrisy, jealousy, faith, fidelity, family, marriage, society, progress, carnal desire and passion, and the agrarian connection to land in contrast to the lifestyles of the city.",
        category: "Realist novel",
        image: "./img/Anna_Karenina_poster.jpg",
      },
      {
        id: 8,
        title: "Les Misérables ",
        price: 10.99,
        description:
          "In Les Misérables Victor Hugo takes readers deep into the Parisian underworld, immerses them in a battle between good and evil, and carries them onto the barricades during the uprising of 1832.",
        category: "historical novel, drama",
        image: "./img/Les-miserables-movie-poster1.jpg",
      },
      {
        id: 9,
        title: "Charlotte Brontë",
        price: 64,
        description:
          "Charlotte Brontë was an English novelist best known for Jane Eyre (1847), the story of an independent young governess who overcomes hardships while remaining true to her principles. It blended moral realism with Gothic",
        category: "Fiction, Novel",
        image: "./img/Charlotte Brontë.jpg",
      },
      {
        id: 10,
        title: "Pride and Prejudice",
        price: 109,
        description:
          "At its core, Pride and Prejudice tells the love story of Elizabeth Bennet and Mr. Darcy, both of whom have to overcome their biases in order to end up together. Throughout the novel, both characters learn to unlearn their pride and prejudice so that they can come to accept the other's goodness of character",
        category: "artistic fiction",
        image: "./img/Pride and Prejudice.jpg",
      },
      {
        id: 11,
        title:
          "The Haunted House",
        price: 109,
        description:
          "For this work, commissioned for his periodical All the Year Round, Dickens enlisted some of the era's most famous writers, including Elizabeth Gaskell and Wilkie Collins, to collaborate with him on this Victorian supernatural classic.",
        category: "artistic fiction",
        image: "./img/The Haunted House.jpg",
      },
      {
        id: 12,
        title:
          "Middlemarch",
        price: 114,
        description:
          "Set in Middlemarch, a fictional English Midland town, in 1829 to 1832, it follows distinct, intersecting stories with many characters.",
        category: "historical genre",
        image: "./img/Middlemarch.jpg",
      },
      {
        id: 13,
        title: "Tess of the d'Urbervill",
        price: 599,
        description:
          " English novel, even Hardy's fictional masterpiece, Tess of the d'Urbervilles received mixed reviews when it first appeared, in part because it challenged the sexual morals of late Victorian England. Tess was portrayed as a fighter not only for her rights, but also for the rights of others.",
        category: "English novel",
        image: "./img/Tess of the d'Urbervill.jpg",
      },
      {
        id: 14,
        title:
          "Mill on the Floss ",
        price: 999.99,
        description:
          "a novel (1860) by George Eliot. It is mainly about the relationship between a brother and sister, Tom and Maggie Tulliver, who live in a mill (= a building where grain is ground into flour) on the river Floss. It describes their childhood and a dispute that causes them to separate.",
        category: "Romance, Love story, Psychological realism",
        image: "./img/Mill on the Floss.jpg",
      },
      {
        id: 15,
        title: "far from the madding crowd",
        price: 56.99,
        description:
          "he novel is set in Thomas Hardy's Wessex in rural southwest England, as had been his earlier Under the Greenwood Tree. It deals in themes of love, honour and betrayal, against a backdrop of the seemingly idyllic, but often harsh, realities of a farming community in Victorian England. It describes the life and relationships of Bathsheba Everdene with her lonely neighbour William Boldwood, the faithful shepherd Gabriel Oak, and the thriftless soldier Sergeant Troy.",
        category: "Romance, Love story, Psychological realism",
        image: "./img/far from the madding crowd.jpg",
      },
      {
        id: 16,
        title:
          "When Stars Fall",
        price: 29.95,
        description:
          "Overall Rating: When The Stars Fall, book one in the Lost Stars Series by Emery Rose is a heartwrenching, heartbreakingly beautiful, slow burn, contemporary romance about a first love that endures a lifetime of trials and tribulations",
        category: "Romance",
        image: "./img/when stars fall.jpg",
      },
      {
        id: 17,
        title: "The Picture of Dorian Gray",
        price: 39.99,
        description:
          "A radiantly handsome, impressionable, and wealthy young gentleman, whose portrait the artist Basil Hallward paints. Under the influence of Lord Henry Wotton, Dorian becomes extremely concerned with the transience of his beauty and begins to pursue his own pleasure above all else.",
        category: "horror",
        image: "./img/Dorian_Grey_-_2009_-_poster.png",
      },
      {
        id: 18,
        title: "The Phantom of the Opera ",
        price: 9.85,
        description:
          "A young soprano becomes the obsession of a disfigured and murderous musical genius who lives beneath the Paris Opéra House. Begins when an opera ghost terrorizes the cast and crew of the French Opera House while tutoring a chorus girl. He finally drives the lead soprano crazy so she and her friend leave.",
        category: "horror",
        image: "./img/opera.jpg",
      },
      {
        id: 19,
        title: "The strange case of Dr. Jekyll and Mr. Hyde",
        price: 7.95,
        description:
          "Strange Case of Dr Jekyll and Mr Hyde by Robert Louis Stevenson is a narrative about the complexities of science and the duplicity of human nature",
        category: "Horror Literature, Gothic Literature, Novel,",
        image: "./img/The strange case of Dr. Jekyll and Mr. Hydejpg.jpg.crdownload",
      },
      {
        id: 20,
        title: "The Shawshank Redemption ",
        price: 12.99,
        description:
          "The Shawshank Redemption is a 1994 American drama film written and directed by Frank Darabont, based on the 1982 Stephen King novella Rita Hayworth and Shawshank Redemption.",
        category: "horror literature",
        image: "./img/shawshank .jpg",
      },
=======
        rname: "Лимонад Simple banana",
        imgdata: "./img/banan.jpg",
        address: "Frash ",
        price: '200',
        rating: "3.8",
        arrimg: "https://www.complexbar.ru/recipe/limonady",
        qnty:0
    },
    {
        id: 2,
        rname: "Тропический клауди ",
        imgdata: "./img/cloudy.jpg",
        address: "Frash ",
       
        price: 200,
        rating: "3.9",
        arrimg: "https://www.complexbar.ru/recipe/limonady",
         qnty:0
    },
    {
        id: 3,
        rname: "Клубнично-грейпфрутовый лимонад",
        imgdata: "./img/430a811c3c1a697f0154c519ff1ca844.jpg",
        address: "Frash ",
        price: 70,
        rating: "4.2",
        arrimg: "https://www.complexbar.ru/recipe/limonady",        
         qnty:0
    },
    {
        id: 4,
        rname: "Лимонад с барбадосской вишней и манго",
        imgdata: "./img/cherry.jpg",
        address: "Frash ",
        price: 70,
        rating: "3.8",
        arrimg: "https://www.complexbar.ru/recipe/limonady",         
        qnty:0
    },
    {
        id: 5,
        rname: "Зелёный безалкогольный физз ",
        imgdata: "./img/green.jpg",
        address: "Frash ",
        price: 210,
        rating: "4.0",
        arrimg: "https://www.complexbar.ru/recipe/limonady",
         qnty:0
    },
    {
        id: 6,
        rname: "Anjoy Latenight Meals",
        imgdata: "./img/0d60505c14a30a58c6105f719b9d5579.jpg",
        address: "Wraps FastFood, Chines",
      
        price: 200,
        rating: "3.8",
        arrimg: "https://www.complexbar.ru/recipe/limonadyp",
         qnty:0
    },
    {
        id: 7,
        rname: "Hocco Eatery",
        imgdata: "./img/gum.jpg",
        address: "Frash ",
        price: 300,
        rating: "3.8",
        arrimg: "https://www.complexbar.ru/recipe/limonady",
         qnty:0
    },
    {
        id: 8,
        rname: "Chai Wai",
        imgdata: "https://www.complexbar.ru/recipe/limonady",
        address: "Tea, Coffee, Shake, Beverages",
        delimg: "https://www.complexbar.ru/recipe/limonady",
        somedata: "500 + order placed from here recently",
        price: 100,
        rating: "3.2",
        arrimg: "https://b.zmtcdn.com/data/o2_assets/4bf016f32f05d26242cea342f30d47a31595763089.png?output-format=webp",
         qnty:0
    },
    {
        id: 9,
        rname: "HL Frankie",
        imgdata: "./img/430a811c3c1a697f0154c519ff1ca844.jpg",
        address: "Burger, Sandwich, Fast Food",
        delimg: "https://www.complexbar.ru/recipe/limonady",
        somedata: "2525 + order placed from here recently",
        price: 100,
        rating: "3.8",
        arrimg: "https://www.complexbar.ru/recipe/limonady",
         qnty:0
    },
>>>>>>> f235992330836ec45f5ef7030f3e7385f048024d
];

export default Cardsdata;